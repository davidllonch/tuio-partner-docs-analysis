...
export function DeclarationTemplatesPage() {
  ...
  const handleUpload = (providerType: string, file: File) => {
    if (file.type !== 'application/pdf') {
      toast({ title: t('declarationTemplates.errorNotPdf'), variant: 'error' })
      return
    }
  ...
  const templateMap = Object.fromEntries(
    (data?.templates ?? []).map((tmpl) => [tmpl.provider_type, tmpl])
  )
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {PROVIDER_TYPES.map(({ value: providerType, label }) => {
              const template = templateMap[providerType]
              const isUploading = uploadMutation.isPending && uploadMutation.variables?.providerType === providerType

              return (
                <div key={providerType} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-shrink-0">
                      <input
                        ref={(el) => { fileInputRefs.current[providerType] = el }}
                        type="file"
                        accept=".pdf"
                        className="hidden"
                        onChange={(e) => {
                          const file = e.target.files?.[0]
                          if (file) handleUpload(providerType, file)
                          e.target.value = ''
                        }}
                      />
                      <button
                        onClick={() => fileInputRefs.current[providerType]?.click()}
                      ...
                      >
                        ...
                        {template ? t('declarationTemplates.replacePdf') : t('declarationTemplates.uploadPdf')}
                      </button>
                    </div>
                  </div>
                </div>
